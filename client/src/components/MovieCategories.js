    ...
    const fetchMovies = async () => {
      try {
        ...
        const responses = await Promise.all(
          categories.map(cat =>
            axios.get(`https://api.themoviedb.org/3/movie/${cat.endpoint}?api_key=${API_KEY}&language=ja-JP&page=1&region=JP`)
          )
        );
        ...
        categories.forEach((cat, index) => {
          let movies = responses[index].data.results;

          // 「公開予定」は今日より後の公開日だけに絞る
          if (cat.id === "upcoming") {
            movies = movies.filter(movie => {
              if (!movie.release_date) return false; // 日付ないのは除外
          ...
          newMovies[cat.id] = movies;
        });

        console.log("取得した映画データ（絞り込み後）:", newMovies); // ✅ APIデータ確認ログ
        setMovies(newMovies);
